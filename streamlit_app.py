...
def analyze(month_num, period_data, turnover_data):
    ...
    start_df = period_data.get(month_num, {}).get('start', pd.DataFrame())
    end_df = period_data.get(month_num, {}).get('end', pd.DataFrame())
    if len(start_df) == 0:
        st.error("未找到该月数据"); return None
    
    start_cnt = len(start_df)
    end_cnt = len(end_df) if len(end_df) > 0 else start_cnt
    ...
    if len(month_turn) > 0 and '一级组织' in month_turn.columns:
        ...
        d2 = start_df.groupby('一级组织').size().reset_index(name='期初人数') if '一级组织' in start_df.columns else pd.DataFrame()
        d3 = end_df.groupby('一级组织').size().reset_index(name='期末人数') if len(end_df) > 0 and '一级组织' in end_df.columns else pd.DataFrame()
        dept_df = d1.copy()
    ...
    for sh in xlsx.sheet_names:
        try:
            ...
            if sh == "离职数据":
                ...
                if mn not in period: period[mn] = {"start": None, "end": None}
                if "期末" in sh:
                    period[mn]["end"] = df
                else:
                    period[mn]["start"] = df
        ...
        if st.button("分析"):
            ...
            if r:
                ...
                if st.button("导出Excel"):
                    ...
                    st.download_button("下载", out.getvalue(), r['month']+"离职分析.xlsx", mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
