...
var chartTurnoverRate, chartTurnoverType, chartTenure, chartReason, chartLevel;
        ...
        .then(function(data) {
            ...
            if (data.success) {
                document.getElementById('uploadStatus').innerHTML = 
                    '<p style="color:green;"><i class="bi bi-check-circle"></i> ' + data.message + '</p>' +
                    '<p>检测到月份: ' + data.summary.available_months.join(', ') + '</p>';
                loadMonths(data.summary.available_months);
            } else {
                document.getElementById('uploadStatus').innerHTML = 
                    '<p style="color:red;"><i class="bi bi-x-circle"></i> ' + data.message + '</p>';
            }
        })
        .catch(function(err) { 
            hideLoading(); 
            document.getElementById('uploadStatus').innerHTML = 
                '<p style="color:red;"><i class="bi bi-x-circle"></i> 上传失败: ' + err.message + '</p>'; 
        });
}
...
function loadMonths(months) {
    ...
    if (months.length > 0) loadMonthData();
}
...
function renderCharts(data) {
    ...
    if (chartTurnoverRate) chartTurnoverRate.dispose();
    chartTurnoverRate = echarts.init(document.getElementById('chartTurnoverRate'));
    chartTurnoverRate.setOption({
        tooltip: { 
            trigger: 'axis',
            formatter: function(params) {
                var dept = params[0].name;
                var count = params[0].value;
                var rate = params[1] ? params[1].value : 0;
                return dept + '<br/>离职人数: ' + count + '<br/>离职率: ' + rate + '%';
            }
        },
        legend: { data: ['离职人数', '离职率'] },
        xAxis: { 
            type: 'category', 
            data: deptData.map(function(d) { return d['一级组织']; }),
            axisLabel: { rotate: 30, interval: 0 }
        },
        yAxis: [
            { type: 'value', name: '离职人数', position: 'left' },
            { type: 'value', name: '离职率(%)', position: 'right', max: 100, axisLabel: { formatter: '{value}%' } }
        ],
        series: [
            { name: '离职人数', type: 'bar', data: deptData.map(function(d) { return d['离职人数']; }), label: { show: true, position: 'top' }, itemStyle: { color: '#2a5298' } },
            { name: '离职率', type: 'line', yAxisIndex: 1, data: deptData.map(function(d) { return d['离职率(%)'] || 0; }), label: { show: true, formatter: '{c}%' }, itemStyle: { color: '#ff6b6b' }, lineStyle: { width: 2 }, symbol: 'circle' }
        ]
    });

    var typeData = data.turnover_type || [];
    if (chartTurnoverType) chartTurnoverType.dispose();
    chartTurnoverType = echarts.init(document.getElementById('chartTurnoverType'));
    chartTurnoverType.setOption({
    ...
    chartReason.setOption({
        ...
        series: [{ type: 'bar', data: reasonData.map(function(d) { return d['人数']; }), label: { show: true, position: 'top' }, itemStyle: { color: '#1e3c72' } }]
    });
    ...
    chartLevel.setOption({
        ...
        yAxis: { type: 'value', name: '人数' },
        series: [{ type: 'bar', data: levelData.map(function(d) { return d['人数']; }), label: { show: true, position: 'top' }, itemStyle: { color: '#28a745' } }]
    });
}
...
window.addEventListener('resize', function() {
    if (chartTurnoverRate) chartTurnoverRate.resize();
    if (chartTurnoverType) chartTurnoverType.resize();
